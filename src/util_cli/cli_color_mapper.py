...
ESC_MAP = {
    RichStyle.COLOR: "ESC[38;2;R;G;Bm",
...
class ColorMapper:
    ...
    def config(self) -> dict:
        ...
        _colors = []
        for i in range(256):
            _colors.append({CODE: i, NAME: COLOR_NAMES[i], HEX: self._hexcolors[i], RGB: self._rgb_colors[i]})
        out[COLORS] = _colors
    ...
    def theme(self, theme: str):
        ...
        if self._themes or theme is None:
            ...
            logger.warning("[ColorMapper] Configuration file was not found")
    ...
    def convert(self, value: Any, to: str = HEX) -> Any:
        ...
        try:
            ...
            if not to == _from:
                logger.warning(
                    f"[ColorMapper] Conversion failed for value [{value}] from [{_from}] to [{to}], allowed {list(self._code_map.keys())}"
                )
                return None
            return value
...
class ThemeConsole(ColorMapper):
    ...
    def __init__(
        self, theme: str = None, color_system: str = "256", create_themes: bool = False, p_resources=None
    ) -> None:
        """Constructor.
        ...
        _theme_env_default = os.environ.get(
            C.ConfigBootstrap.CLI_DEFAULT_THEME.name, C.ConfigBootstrap.CLI_DEFAULT_THEME.value
        )
        ...
        if theme is None:
            ...
            theme = _theme_env
            logger.debug(
                f"[ThemeConsole] Setting Theme from ENV/ENV_DEFAULT [{_theme_env}/{_theme_env_default}] to [{theme}]"
            )
        ...
        if theme in self._themes:
            ...
            logger.warning(
                f"[ThemeConsole] Invalid Theme [{theme}], allowed {self._themes},using default [{self._default_theme}]"
            )
    ...
    def _set_rich_themes_path(self):
        ...
        if isinstance(self._p_resources, str) and os.path.isdir(self._p_resources):
            ...
            print(
                "[ThemeConsole] Couldn't create path to themes folder, create theme first by calling ThemeConsole(create_themes=True)"
            )
    ...
    def get_console(self, theme: str = None, force_terminal: bool = None) -> Console:
        ...
        if not _theme_name in self._themes:
            logging.debug("[ThemeConsole] No theme was transferred, using initital theme")
