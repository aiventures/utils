...
from util.datetime_util import DateTimeUtil, WEEKDAY
...
class CalendarFilter:
    ...
    def __init__(self,filter_s:str=None,date_list:List[List[DateTime]|DateTime]=None):
        ...
        self._parse_func = {CalendarRegex.REGEX_NOW:self._parse_now,
    ...
    def parse_yyyymmdd(self,*args)->DateTime:
        ...
        _dates,_date = args
        if len(_dates) != 1:
            logger.warning(f"[CalendarFilter] [{args}] Regex Hit {_dates} should only contain one date (REGEX_YYYYMMDD)")
            return None
        try:
