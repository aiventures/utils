...
class DictFilter:
    ...
    def add_filter(self, filter_value, **kwargs):
        ...
        # default filter dict
        filter_dict = {Filter.VALUE: filter_value, Filter.TYPE: Filter.CONTAINS, Filter.OBJECT: Filter.KEY}
        ...
        logger.debug(
            f"[DictFilter] Adding Filter for [{filter_object}][{filter_type}]:({filter_value}), [{len(filter_dict)-3}] additional params"
        )
    ...
    def filter(self, info_dict: dict, verbose: bool = False) -> bool:
        ...
        for object_filter in self._filters:
            ...
            if verbose:
                ...
                logger.debug(
                    f"[DictFilter] FILTER OBJECT [{filter_object.name}] {filtered_object} {filter_type.name} {filter_value}, level pass: {level_passed}"
                )
    ...
    def _itemized_dict(self, d: dict, parent_id):
        ...
        for k, v in d.copy().items():
            ...
            if parent_id:
                ...
                self._hierarchy[obj_id] = {"parent": parent_id, "key": k, "object": v, "obj_type": v_type, "id": obj_id}
            else:
                obj_id = None
...
if __name__ == "__main__":
    ...
    if True:
        test_struc = (
            '{"k1":"value1",' '"test_key":500,' '"k2":{"k2.1":5,"k2.2":"v2.2",' '"k2.3":["l1","test value","l3"]}}'
        )
