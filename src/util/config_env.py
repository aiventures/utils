...
class ConfigEnv:
    ...
    def _validate_command(self, key: str) -> bool:
        ...
        for _param_name, _param_type in _params.items():
            if not _param_name in _used_params:
                _msg = (
                    f"[CONFIG] Key [{key}], Param [{_param_name}], is not used in any of the variables {_used_params}"
                )
                logger.warning(_msg)
                valid = None
    ...
    def _resolve_env(self, key: str) -> bool:
        ...
        if C.ConfigAttribute.PATH.value in _attributes or C.ConfigAttribute.FILE.value in _attributes:
            _resolved = self._resolve_path_object(key)
    ...
    def _subst_dependencies(self, key: str, dependencies: dict) -> bool:
        ...
        for _att in _attributes:
            ...
            if "[" in _config_value:
                logger.warning(
                    f"[ConfigEnv] Error in Key [{key}], [{_att.name}], original [{_config_value_original}], resolved to [{_config_value}] "
                )
                return False
...
class Environment:
    ...
    def create_set_vars_bat(
        self, bat_env_dict: dict = None, f_out: str = None, bat_set_list: list = None, bat_echo_list: list = None
    ) -> str:
        ...
        for _, _info in _env_dict.items():
            ...
            if _key:
                ...
                if _value:
                    ...
                    logger.warning(
                        f"[ENV] Can't Set Environment for config key/key/value [{_config_key}/{_key}]:[{_value}]"
                    )
    ...
    def bootstrap_config(
        ref: str, config_env: ConfigEnv = None, origin_list: List[str] = [], cwd: str = None
        ...
        for _ref_source, _ref_value in _config.items():
            ...
            _source_ref = SourceRef(
                key=ref, value=_ref_value, ref_type=_ref_type, is_file=_is_file, is_path=_is_path, cwd=cwd
            )
    ...
    def bootstrap_ref(
        ...
        for _ref_source in origin_list:
            ...
            elif _ref_type == SourceEnum.PATH or _ref_type == SourceEnum.FILE:
                _value = ref
            ...
            # strict validation
            if _value is not None and strict and (_is_file is False and _is_path is False):
                _value = None

            _source_ref = SourceRef(
                key=ref, value=_value, ref_type=_ref_type, is_file=_is_file, is_path=_is_path, cwd=_cwd
            )
