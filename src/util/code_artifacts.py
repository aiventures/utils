...
ARTIFACT_FILTER = {
    ARTIFACT.GIT: {"include_paths": ".git$", "paths_only": True},
...
class CodeArtifact(ABC):
    ...
    def __init__(self, artifact_meta: ArtifactMeta = ArtifactMeta()):
        ...
        if artifact_type is not None:
            ...
            logger.warning(
                f"[CodeArtifact] No Filter Dict or Artifact type {list(ARTIFACT_FILTER.keys())} was transferred"
            )
            return
...
class VenvArtifact(CodeArtifact):
    ...
    def read_meta(p_venv: str) -> None | VenvMeta:
        ...
        _package_list = [
            Path(_p).name
            for _p in os.listdir(_path_site_packages)
            if os.path.isdir(os.path.join(_path_site_packages, _p))
        ]
...
ARTIFACT_CLASS = {ARTIFACT.GIT: GitArtifact, ARTIFACT.VSCODE: VsCodeArtifact, ARTIFACT.VENV: VenvArtifact}
...
class CodeArtifacts:
    ...
    def link_code_meta(self) -> CodeMetaDict:
        ...
        for _p_git, _git_meta in _git.info_dict.items():
            ...
            code_meta_dict[_p_git] = CodeMeta(
                p_git=_p_git,
