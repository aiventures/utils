...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "CMD_EXAMPLE1", {"file": "[CMD_EXAMPLE1]", "line": 6}, id="A.02 CMD Parsing with a Config Reference"
        ),
    ],
)
...
def create_testcases_get_ref() -> list:
    ...
    out.append(
        pytest.param(
            True, True, str(_ref_cmd_valid_anycase), id="C.09 valid Where Ref case insensitive test [w_pYTHoN]"
        )
    )
    out.append(
        pytest.param(
            False, False, str(_ref_valid_invalid_fileinfo), id="C.10 valid Ref with invalid Path Ref [P_30_ENTWICKLUNG]"
        )
    )
    out.append(
        pytest.param(
            False, False, str(_f_testfile_validroot_invalidfile), id="C.11 valid root path with invalid filename"
        )
    )
